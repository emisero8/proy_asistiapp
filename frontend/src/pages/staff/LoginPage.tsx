...
export function StaffLoginPage() {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (session.rol !== "Staff_QR" && session.rol !== "Staff_Vendedor") {
        setError("Esta cuenta no es de Staff.");
        return;
      }
      login(session);
      navigate(session.rol === "Staff_QR" ? "/staff/scanner" : "/staff/pos");
    } catch (e: unknown) {
      ...
      <div className="w-full max-w-sm relative bg-card/60 border border-border rounded-3xl p-6 sm:p-8">
        ...
        >
          ...
          <button
            ...
            className="w-full py-4 rounded-2xl bg-primary text-primary-foreground font-bold text-base hover:bg-primary/90 hover:scale-[1.015] active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
